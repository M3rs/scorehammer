        ...
        <section id="scores">
            ...
            <tbody>
              {[...Array(5).keys()].map((v, i) => {
                return (
                  <tr key={i}>
                    <td>{i + 1}</td>
                    <td><input type="number" step="5" onChange={e => updateP1Score(i, e)} pattern="\d*" /></td>
                    <td><input type="number" step="5" onChange={e => updateP2Score(i, e)} pattern="\d*" /></td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <div className="secondary">
            <div className="secondaryList">
              <div>{p1} {p1Faction}</div>
              {p1Secondaries.map((s, i) => {
                return (
                  <div>
                    <div>{s}</div>
                    <input type="number" onChange={e => updateP1Score(5 + i, e)} pattern="\d*" />
                  </div>
                );
              })}
            </div>
            <div className="secondaryList">
              <div>{p2} {p2Faction}</div>
              {p2Secondaries.map((s, i) => {
                return (
                  <div>
                    <div>{s}</div>
                    <input type="number" onChange={e => updateP2Score(5 + i, e)} pattern="\d*" />
                  </div>
                );
              })}
            </div>
